...
if( $get_tab=='gift' ) {
	...
	if($oneself){
		...
		$item_html .= '<li class="tip">' . __('请注意保管你的唯一识别码，这是兑换礼品的身份凭证。','dmeng') . '</li>';
		...
		if($giftLog){
			foreach( $giftLog as $log ){
				...
				$item_html .= '<li>'.$log->meta_value.' <span class="message-content">'.sprintf( __('兑换了%1$s，唯一识别码是 %2$s ', 'dmeng'), get_the_title($gift_id[1]), dmeng_get_exchange_key($curauth->ID, $gift_id[1], $log->meta_value) ).'</span></li>';
			}
...
<form id="info-form" class="form-horizontal" role="form" method="post">
	...
	<div class="form-group">
		<label class="col-sm-3 control-label"><?php _e('用户ID','dmeng');?></label>
		<div class="col-sm-9">
