...
<div id="main" class="container">
	<div class="row">
	<div id="content" class="col-lg-12 col-md-12 archive" role="main" itemscope itemprop="mainContentOfPage" itemtype="http://schema.org/Blog">
<?php
...
if( $paged<2 ){
					...
					$headline = '<article id="post-'.get_the_ID().'" class="panel panel-default panel-headline archive" data-post-id="'.get_the_ID().'" role="article" itemscope itemtype="http://schema.org/Article">';
						$headline .= '<div class="panel-body">';
							$headline .= '<div class="entry-header page-header">';
								$headline .= '<h3 class="entry-title">'.apply_filters( 'dmeng_sticky_the_title', '<a href="'.get_permalink().'" title="'.$title.'" rel="bookmark" itemprop="url"><span itemprop="name">'.$title.'</span></a>' ).'</h3>';
							$headline .= '</div>';
	...
	if($home_cat){
		...
		$cat_cache_key = 'dmeng_home_cat_lists';
		...
		if ( false !== $cat_cache ) {
			...
			foreach($home_cat as $cat_id){
				...
				if($category){
					$cat_output .= '<div class="col-lg-'.$cat_col.' col-md-'.$cat_col.' col-sm-'.$cat_col.'"><div class="'.apply_filters('dmeng_home_cat_panel_class', 'panel panel-default home-posts-list').'">';
					...
					query_posts( array( 'ignore_sticky_posts' => 1, 'posts_per_page' => 5, 'cat' => $category->term_id ) );
						while ( have_posts() ) : the_post();
								$cat_output .= apply_filters( 'dmeng_homelist_the_title', '<a href="'.get_permalink().'" title="'.get_the_title().'" rel="bookmark" itemprop="url" class="list-group-item">'.get_the_title().'</a>' );
						endwhile;
					wp_reset_query();
...
if($home_post){
	...
	echo '<div class="'.apply_filters('dmeng_archive_panel_class', 'panel panel-default panel-archive').'" role="main"><div class="panel-body">';
