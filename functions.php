 ...
 * 调用分类模板
 */
get_template_part('category', 'func'); 
...
/**
 * 输出CSS或者JS
 * @param  string $name  CSS或JS名称
 * @param  string $speed 是否使用CDN
 * @return [type]        URL
 */
function dmeng_script_uri( $name = '' , $speed = 'auto' ){
	...
	switch( $name ){
		...
		// 百度ECharts
		case 'echarts'	:
			$path = '/js/echarts.min.js';
			break;

		case 'bootstrap_css' :
			$path = $speed ? '/bootstrap/3.3.6/css/bootstrap.min.css' : '/css/bootstrap.min.css';
			break;
			
		case 'bootstrap_js' :
			$path = $speed ? '/bootstrap/3.3.6/js/bootstrap.min.js' : '/js/bootstrap.min.js';
			break;
...
function dmeng_enqueue_scripts(){
	...
	wp_register_script( 'bootstrap',  dmeng_script_uri('bootstrap_js'), array( 'jquery' ), '3.3.6' );
	// wp_register_script 第三个参数为脚本依赖包，预先加载
	wp_register_script( 'dmeng',  dmeng_script_uri('js'), array( 'jquery', 'bootstrap' ), '2.0.9.5' );
	// 注册 CIE 和 ECharts 脚本
	wp_register_script( 'echarts',  dmeng_script_uri('echarts'), array(), '3.1' );	
	wp_register_script( 'cie',  dmeng_script_uri('cie_js'), array('echarts'), '1.0' );
	wp_register_script( 'google_code_prettify',  dmeng_script_uri('prettify'), array(), '4-Mar-2013' );
	
	// 注册CSS
	wp_register_style( 'bootstrap',  dmeng_script_uri('bootstrap_css'), array(), '3.3.6' );
	wp_register_style( 'dmeng',  dmeng_script_uri('css'), array( 'bootstrap' ), '2.0.9.5' );

	wp_enqueue_script('dmeng');
	wp_enqueue_script('cie');
...
function dmeng_get_the_thumbnail( $size = 'thumbnail' ) {
...
function dmeng_widgets_init() {
	...
	register_sidebar( array(
		...
		'before_widget' => '<div class="col-xs-3 col-sm-3 col-md-3 col-lg-3"><aside id="%1$s" class="widget clearfix footer-widget %2$s">',
...
function dmeng_post_meta_lite(){
	...
	$output .= apply_filters('dmeng_post_meta_date', '<span class="glyphicon glyphicon-calendar" style="margin-right:0.5em"></span><time class="entry-date" title="'.sprintf( __('发布于 %1$s 最后编辑于 %2$s ', 'dmeng'), get_the_time('Y-m-d H:i:s'), get_the_modified_time('Y-m-d H:i:s') ).'" datetime="'.get_the_date( 'c' ).'"  itemprop="datePublished">'.get_the_date().'</time>');
	...
	if( get_post_type()=='post' ) {
		...
		if( apply_filters('dmeng_post_meta_cat_show', true) ){
			...
			if($categories){
				...
				$output .= apply_filters('dmeng_post_meta_cat', '<span class="glyphicon glyphicon-folder-open" style="margin-right:0.5em;margin-left:0.5em"></span>' . join(' | ',$cats) );
			}
		}
...
function dmeng_post_footer(){
	...
	echo '<div class="entry-footer clearfix" role="toolbar">';
		...
		dmeng_breadcrumb_html(get_the_ID(),' › ');
...
function pagination($pages = '', $range = 4)
{
     ...
     if(1 != $pages)
     {
         echo "<div class=\"pagination\"><ul><li><span>第".$paged."页 共".$pages."页</span>";
         if($paged > 2 && $paged > $range+1 && $showitems < $pages) echo "<a href='".get_pagenum_link(1)."'>&laquo; 首页</a>";
         if($paged > 1 && $showitems < $pages) echo "<a href='".get_pagenum_link($paged - 1)."'>&lsaquo; 前一页 &nbsp;</a>";
         ...
         if ($paged < $pages-1 &&  $paged+$range-1 < $pages && $showitems < $pages) echo "<a href='".get_pagenum_link($pages)."'>末页 &raquo;</a>";
         echo "</li></ul></div>\n";
     }
}